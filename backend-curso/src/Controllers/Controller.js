...
module.exports = {
  ...
  async transaction(req, res) {
    ...
    const mailchimpData = {
      ...
      status: 'subscribed',
      tags: ['curso_visitou', 'curso_inscrito'],
      merge_fields: {
        NOME_COMP: data.nome,
    ...
    const emailData = {
      ...
      STATUS: 'Aguardando',
      CODIGO: '',
      PREVIEWTEXT: 'Nova Inscrição Curso Fitoterapia',
    };

    if (data.paymentMethod === 'debt') {
      mailchimpData.tags.push('curso_pag_offline');
      ...
      if (result.status) {
        ...
        const email = {
          ...
          Template: 'Admin-Inscricao-Curso-v1-0-0',
    ...
    if (status !== 7) {
      ...
      if (status === 3) {
        mailchimpData.tags.push('ws_pag_efetuado');
      } else {
        mailchimpData.tags.push('ws_pag_pendente');
      }
